...
**CORRECTION (this section originally contained a false finding — see below).** An earlier pass of this spec claimed the header does not collapse to a mobile menu at 390px and wraps badly. That claim was based on a screenshot taken via a browser-automation `resize_window` call whose reported pixel dimensions did not match the page's actual `window.innerWidth` at the time (confirmed via direct JS query: a requested 390px resize was actually rendering the page at ~606px CSS width, not 390px — an automation-tool artifact, not a real page-rendering issue). Re-tested with the real viewport width verified by `window.innerWidth` before every screenshot. The corrected finding is below; the false one is documented here rather than silently deleted, since a reconstruction spec that quietly reverses itself is worse than one that shows its work.

**OBSERVED, desktop (1456×828 and 1568×775 viewports):** page renders as described throughout §2, no horizontal overflow, all six images load and render correctly, 0 console errors on load.

**OBSERVED, real mobile-width viewport (606×667, confirmed via `window.innerWidth` immediately before screenshotting — see `site-header.tsx`: the desktop `<nav>` is `hidden lg:block`, the mobile disclosure is `className="lg:hidden"`, so anything below the 1024px `lg:` breakpoint is "mobile" for this header regardless of exact width):** the header correctly collapses to a compact bar — wordmark, theme toggle, and a single hamburger (`<details>`-based disclosure) button. Clicking it opens a full-height native `<details>` panel listing every primary nav item, with dropdown children (e.g. CDT-2's 7 section links) shown inline and expanded rather than as a second level of disclosure, and the current page highlighted. No wrapping, no overflow, no console errors. This is a well-built, correctly-functioning mobile nav — the opposite of the originally-reported defect.

**INFERRED (not directly screenshotted at every section below the hero):** given every section below the hero uses the same `.oxot-canvas` responsive padding and standard Tailwind grid breakpoints (`sm:`/`md:`/`lg:`), and given this session's `measure.mjs` gate (which checks 390/834/1440/2560px for horizontal overflow) has passed clean on this route repeatedly earlier in the session, section content itself is expected to stack cleanly to single-column below their respective breakpoints without overflow — but this was not re-screenshotted section-by-section in this pass, so treat as inferred, not directly observed, for anything past the hero.
...
9. The mobile nav's `lg:1024px` collapse behavior (§9) is preserved as-is — it works correctly and is inherited from `SiteHeader`, not something this page's own rebuild should touch.
