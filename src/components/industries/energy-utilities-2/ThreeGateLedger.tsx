...
import {
  assetLabel, BASELINE, CANVAS, CANVAS_ASSETS, CITATION, CONTROL_CANDIDATES, CONTROLS,
  ENTRY_ASSET_ID, EVIDENCE_GROUPS, INTERMEDIATE_IDS, NODE_H, NODE_POS, NODE_TEXT_W, NODE_W,
  OUTPUT, RESULT, ROUTE_POINTS, routeLabel, SCENARIO_PATHS, SCENARIO_PROSE, SECTION,
  STATUS_SWATCH, TARGET_ASSET_ID, VIEWBOX, wrapLabel, type ControlCandidate
} from "./content.workedExample";

/**
 * S06 · PATTERN 2 — THE THREE-GATE LEDGER, energy application. The three defects
 * `OXOT_Layout_Styles.md` records as having shipped live on earlier Pattern 2
 * builds are designed out here rather than fixed afterwards.
 *
 ...
 *     `<details>`/`<summary>` rather than an always-open `<dl>` — the same fix
 *     `water-wastewater-2` and `-3` carry. This column is narrowest by mandate
 *     and densest by design, so narrow-and-tall is a structural failure mode,
 *     not an accident; shortening the DEFAULT pre-interaction state fixes it
 *     without widening the column out of its stated order. NOT applied to the
 *     other two: Baseline is the reader's entry point, and Proposed control is
 *     governed by its own minimum-density floor, where hiding content is the
 ...
 * inferred from the routes it closes. "Patch or upgrade component" closes no
 * route and is still inserted at the reachable component; a geometric inference
 * would have left that candidate marking nothing. See content.workedExample.ts.
 *
 ...
 * `:focus-visible` box-shadow, and Tailwind's ring utilities overwrite it with a
 * transparent scaffold — measured on a sibling page as `:focus-visible` matching
 * with no visible indicator at all (WCAG 2.4.7).
 */
...
export function ThreeGateLedger({ locale, className }: { locale: Locale; className?: string }) {
  ...
  /* NO `useMemo` ON EITHER DERIVATION BELOW. The React Compiler is enabled in
     ...
     computations are one pass over eight routes, so hand-memoizing them buys
  ...
  if (control) {
    ...
    /* Named by ROUTE, not by destination: several of this scenario's routes end
       at the same node, and labelling by target would print the same chip
    ...
    >
      ...
      <figure className="m-0 mt-10" data-gfx-meaning>
        <div className="overflow-x-auto rounded-2xl border border-border bg-card p-4 sm:p-5">
          ...
          >
            <defs>
              <PathEdgeMarkerDefs idPrefix="eu2-vendor" />
            </defs>
            {paths.map((path) =>
              ROUTE_POINTS[path.id] ? (
                <PathEdge key={path.id} path={path} points={ROUTE_POINTS[path.id]} markerId={`eu2-vendor-${path.status}`} />
              ) : null
            )}
      ...
      <p className="mt-4 body-copy leading-relaxed text-muted-foreground">
        ...
        >
          {pick(CITATION.linkLabel, locale)}
        </a>
      </p>
    </SectionA>
  );
...
function Node({ asset, highlighted, marked }: { asset: SystemAsset; highlighted: boolean; marked: boolean }) {
  ...
  return (
    <g>
      ...
      {/* `assetGlyph(asset)`, NOT `ASSET_GLYPHS[asset.type]`. Two of this
          canvas's eight nodes are `network-device` — the operational-DMZ jump
          host and the OT firewall / VLAN boundary — and they sit ADJACENT,
          joined by the edge whose crossing is this section's whole finding, so
          the type table drew a host and a boundary as one identical LAN
          topology. The resolver honours the `symbol` slugs
          `content.workedExample.ts` now puts on those two and falls back to the
          type silhouette for the other six, which is what those six should
          draw: nothing else on this canvas repeats a mark.

          CALLED, NEVER BOUND — `{assetGlyph(asset)}` returns an ELEMENT.
          Binding a component from a call (`const G = assetGlyph(asset)`) is
          what `react-hooks/static-components` rejects, because in general a
          call returns a fresh component and React remounts a subtree whose
          element type identity changed. `AssetNode.tsx` memoises per slug, so
          the element form keeps identity stable with nothing to suppress. */}
      <svg x={x + 7} y={y + 10} width={22} height={22} viewBox="0 0 32 32">
        {assetGlyph(asset)}
 ...
 *  a real sentence, not a blank: one candidate closes no route by design, and
 *  that is the source's own finding about it. */
