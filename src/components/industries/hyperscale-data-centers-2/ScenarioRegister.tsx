...
import { SCENARIOS, SCENARIOS_SECTION } from "./content.scenarios";

/**
 * S07 · HYPERSCALE RISK SCENARIOS — a master/detail register whose detail is a
 * DRAWING where the source supports one and a stated gap where it does not.
 * Source L286–L307.
 *
 ...
 * section is TWELVE SCENARIOS × FOUR FIELDS = 48 cells, which clears the cell
 * count but is not the shape the rule is aimed at: these are not categories
 * with rows underneath, they are twelve records of four fields each, with only
 * one record's fields visible at a time. Tabs would additionally promise a
 * separate panel per item when there is ONE shared detail pane that every
 * selection writes into — the same mis-fit the sibling `-2` pages record.
 *
 ...
 * which matters more here than on any sibling, because twelve rows would
 * otherwise cost twelve sequential tab stops before the pane beside them.
 *
 * THE DETAIL PANE IS A LABELLED REGION AND DELIBERATELY NOT A LIVE REGION.
 * `Architecture.tsx` on this page puts `aria-live` on a one-line summary, which
 * is right for one line; announcing this pane on every arrow press would read a
 * heading, a pathway sentence, a whole topology and two captions each time the
 * selection moved by one. The selected radio's own label already announces the
 * change, and the region is reachable as a landmark for the reader who wants
 * the rest.
 *
 ...
 * construction; the pathway, consequence and decision live in the pane, in
 * full, for the selected row. The list is ONE column at every breakpoint, so
 * DOM order, reading order and arrow order are the same order without needing a
 * grid-flow correction.
 *
 * THE PANE STICKS AND THE LIST DOES NOT, and with twelve rows that is
 * load-bearing rather than a nicety: the register column is the taller of the
 * two, so a reader arrowing to the twelfth row would otherwise have scrolled
 * the pane off-screen and be selecting blind.
 *
 * NO LEAD SITS UNDER THE HEADING. The source's L292 is a build instruction
 * ("Use an interactive scenario library…"), not visitor copy — see
 * `SCENARIOS_BUILD_NOTE` in `content.scenarios.ts`, which transcribes it and
 * says why it is honoured rather than printed.
 *
 ...
 *
 * SIBLING BALANCE, `data-balance-group="hyperscale-scenarios"`, site-wide 2x
 ...
 * consequence rule only. A selector is chrome; a scenario register is not a
 * model state.
 */

const BALANCE_GROUP = "hyperscale-scenarios";
      ...
      <div className="grid items-start gap-8 lg:grid-cols-12 lg:gap-10">
        <div className="lg:col-span-5">
          <div data-balance-group={BALANCE_GROUP}>
            <div
              role="radiogroup"
              aria-label={pick(SCENARIOS_SECTION.registerLabel, locale)}
