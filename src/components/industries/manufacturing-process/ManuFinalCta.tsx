 ...
 * every real page — same reasoning as EnergyFinalCta.tsx and /assurance's
 * own closing panel: the source brief's final-CTA copy ("bring a P&ID, an
 * asset list...") is distinct, page-tailored persuasion worth more than the
 * generic three-door choice alone.
 */
export function ManuFinalCta({ locale }: { locale: Locale }) {
  const t = FINAL_CTA;
  /* /technical-specification is EN-only — NL falls back to /cdt-2 (no
     locale guard), same no-404 reasoning as ManuRegulatory's assurance
     link. */
  ...
  return (
    <section aria-labelledby="final-cta" className="mt-16 border-t border-border pt-10">
      <div className="relative">
        <ManuCornerFrame />
        <div className="rounded-2xl border-2 border-border bg-muted p-6 sm:p-8">
