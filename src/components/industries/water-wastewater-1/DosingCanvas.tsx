...
/**
 * THE SHARED CANVAS for the Three-Gate Ledger below it — one drawing all three
 * columns refer to, rather than one visual per column, which is what Pattern 2
 * actually specifies.
 *
 * WHY HAND-AUTHORED GEOMETRY AND NOT `TwinExplorer`. `TwinExplorer` is real,
 * verified and genuinely available, and it is the right tool where a layout has
 * to be computed from data whose shape changes. Here it is the wrong one, for
 * the reason `/decisions/change-safely` already recorded for its own diagram:
 * this canvas re-renders on every control selection, and re-running an ELK
 * layout each time would make the nodes jump, which reads as a live simulation
 * running in the browser — a claim OXOT does not make. Fixed geometry holds the
 * eight nodes still, so the only thing that changes is the thing that actually
 * changed: the state of each route.
 *
 * WHAT IS REUSED RATHER THAN REDRAWN: `PathEdge` and `PathEdgeMarkerDefs` for
 * every edge, so colour still tracks `SystemPath.status` and stroke geometry
 * still tracks `role`, exactly as everywhere else on this site; and
 * `ASSET_GLYPHS` / `CriticalityMark` / `TYPE_LABEL` from `AssetNode`, so the
 * nine asset silhouettes are the same nine the Twin canvas and the bento grid
 * use. No second icon set, no second colour rule.
 *
 * THE CROSSHAIR is Pattern 2's own idiom — a circled cross, P&ID convention,
 * marking where a proposed control is inserted — drawn in `--signal-amber`
 * because amber means proposed/pending. It is never a rounded UI pill.
 *
 * ZONE FRAMES are dashed `--border` outlines with plain labels. They carry the
 * zone/conduit language the source's own control list asks for ("virtual
 * firewall rules and conduit design between vendor path, engineering
 * workstation, SCADA, and dosing PLC") and deliberately use no signal colour,
 * because a zone boundary is structure, not state.
 */
...
export function DosingCanvas({ assets, paths, marks, title, locale }: DosingCanvasProps) {
  return (
    <figure className="m-0">
      <div className="overflow-x-auto rounded-2xl border border-border bg-card p-4 sm:p-5">
        ...
        >
          <defs>
            <PathEdgeMarkerDefs idPrefix="dosing-arrow" />
          </defs>
          ...
          {paths.map((path) => {
            ...
            return <PathEdge key={path.id} path={path} points={points} markerId={`dosing-arrow-${path.status}`} />;
          })}
          ...
          {assets.map((asset) => {
            ...
            return (
              <g key={asset.id}>
                ...
                {lines.map((line, li) => {
                  /* Two of the eight real labels are long enough to run past
                     the node box, and the Dutch translations will be longer
                     still. Rather than drop the font below the 11px technical
                     -label floor, an over-long line is condensed to the
                     available width with SVG's own `textLength` — the type
