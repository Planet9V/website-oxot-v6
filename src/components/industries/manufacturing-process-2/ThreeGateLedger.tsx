...
import {
  assetLabel, BASELINE, CANVAS, CANVAS_ASSETS, CLOSING, CONTROL_CANDIDATES, CONTROLS,
...
/**
 * S06 · PATTERN 2 — THE THREE-GATE LEDGER, manufacturing application. The three
 ...
 * inferred from the routes it closes. TWO of this scenario's five candidates
 * close no route — "Patch immediately" and "Replace the controller" — and a
 * geometric inference would have left both marking nothing at all. See
 * content.workedExample.ts.
 *
...
export function ThreeGateLedger({ locale, className }: { locale: Locale; className?: string }) {
  ...
  /* NO `useMemo` ON EITHER DERIVATION BELOW. The React Compiler is enabled in
     ...
     computations are one pass over seven routes, so hand-memoizing them buys
  ...
  if (control) {
    ...
    /* Named by ROUTE, not by destination: three of this scenario's routes end at
       the same node, and labelling by target would print the same chip
    ...
    >
      ...
      <figure className="m-0 mt-10" data-gfx-meaning>
        <div className="overflow-x-auto rounded-2xl border border-border bg-card p-4 sm:p-5">
          ...
          >
            <defs>
              <PathEdgeMarkerDefs idPrefix="mp2-worked" />
            </defs>
            {paths.map((path) =>
              ROUTE_POINTS[path.id] ? (
                <PathEdge key={path.id} path={path} points={ROUTE_POINTS[path.id]} markerId={`mp2-worked-${path.status}`} />
              ) : null
            )}
            ...
            <div data-balance-group={BALANCE_GROUP}>
              ...
              {control && (
                <div className="mt-6 border-t border-signal-amber/50 pt-4">
                  <Field label={pick(CONTROLS.testsLabel, locale)} value={pick(control.tests, locale)} />
      ...
      <p className="mt-4 body-copy leading-relaxed text-muted-foreground">{pick(CLOSING, locale)}</p>
    </SectionA>
  );
}
  ...
  return (
    <g>
      ...
      {/* `assetGlyph(asset)`, NOT `ASSET_GLYPHS[asset.type]`: two of this
          canvas's eight nodes are `network-device` and drew the same silhouette
          for a session-carrying pathway and a zone boundary. The resolver
          honours the `symbol` slug `content.workedExample.ts` puts on
          `zone-firewall` and falls back to the type silhouette for the other
          seven, which is what the other seven should draw.

          CALLED, NEVER BOUND — `{assetGlyph(asset)}` returns an ELEMENT. Binding
          a component from a call (`const G = assetGlyph(asset)`) is what
          `react-hooks/static-components` rejects, because in general a call
          returns a fresh component and React remounts a subtree whose element
          type identity changed. `AssetNode.tsx` memoises per slug, so the
          element form keeps identity stable with nothing to suppress. */}
 ...
 *  a real sentence, not a blank: two candidates here close no route by design,
 *  and that is the source's own finding about them. */
