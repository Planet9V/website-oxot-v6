...
/**
 * S04 · MANUFACTURING RISK SCENARIOS — a master/detail register whose detail is
 * a DRAWING. Source L141–L160.
 *
 ...
 * section is 8 scenarios × 4 columns = 32 cells, under that threshold, and they
 * are not categories with rows underneath: each scenario is ONE record with
 * four fields, and only one record's fields are ever visible at a time. Tabs
 * would also promise a separate panel per item when there is one shared detail
 * pane that every selection writes into. `TechnologyIndex.tsx` on this same
 * page is what that rule is actually aimed at, and this is not that shape.
 *
 ...
 * both cases, never the grid cells.
 *
...
const REGISTER_LABEL = "Manufacturing and process cyber-risk scenarios";
...
export function ScenarioRegister({ locale }: { locale: Locale }) {
  ...
  return (
    <SectionA
      ...
      locale={locale}
    >
      {/* Source L147's chain, printed as the legend for what the pane below
          shows. Its first two stages are the graph and its last three are the
          graph's captions — stated here so that split reads as a structure
          rather than as a diagram that stopped early. */}
      <div className="rounded-2xl border border-border bg-muted/40 p-5">
        ...
        <p className="mt-4 text-sm leading-relaxed text-muted-foreground">{pick(CHAIN_SPLIT_NOTE, locale)}</p>
      </div>
      ...
      {/* Source L160. Its trailing citation marker is deliberately left
          unresolved rather than pointed at `/technical-specification` — the
          claim is about external industrial guidance, not about OXOT's own
          approach, and the other markers on this page resolve to OXOT's own
          specification. See `content.scenarios.ts`'s docblock. */}
      <div className="mt-12 rounded-2xl border border-border bg-muted/40 p-5">
        <p className="mono-label text-primary-ink">{pick(SCENARIOS_CLOSING.eyebrow, locale)}</p>
