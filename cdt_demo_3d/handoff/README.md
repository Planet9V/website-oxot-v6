...
| --- | --- |
| Facility | Substation, MV/LV, UPS, batteries, generators, BESS, chiller facility, cooling towers, HVAC, MMR, core IT/DCS room, NOC, OT boundary, fire, security, internet boundary |
...
**Totals: 449 assets · 1,155 connections · 1,155 solved routes (17,720 m) · 216 critical conduits · 188 hardwired or serial connections · 5,966 meshes.** Two independent cells (141 / 132 assets, 176 campus-shared), 11 CyHAZOPs hazards (max cyber RPN 294), 15 critical items, 8 costed safeguards, and a $8.88M baseline ALE. Integrity is enforced: 0 danglers, 0 phantom endpoints, 0 duplicate ids, 0 unrouted connections.

**To publish this to the website, follow `EXPORT.md`** — browser GLB export, Blender enhancement, validation, then upload.
...
graph/
  ...
  facility-graph.json   209 assets / 314 connections + IEC 62443 layer
  hall-graph.json       240 assets / 841 connections
  hazard-log.json       11 CyHAZOPs hazards, 15 critical items, 8 safeguards, ALE + ROSI, cell conformance
  ...
  routes.json           1,155 solved runs with point lists and corridor occupancy
  positions.json        449 asset anchors in GLB world space
...
docs/
  oxot-cdt-sample-report.html   sample artifact pack (9 artifacts, 20 rows each)
  oxot-cdt-process.html         phase/workshop process diagram
  oxot-web-copy.html            website copy for the model and the method

kit/
  AUTHORING.md          seven-phase procedure for building the next site
  PROMPT.md             the brief to hand over with a new set of HLD/DLD/ICD

EXPORT.md               publishing procedure — read this to put the model on the site
```
...
2. Click **Download GLB** → `hyperscale_campus.glb` (5,966 meshes, full hierarchy preserved)
...
**This is now built.** See `babylon/hyperscale-campus-explorer.html` and
`babylon/README.md` (deployment, user guide, developer guide, known gotchas — including a
coordinate-system fix this section's pseudocode below doesn't cover). The patterns below are
still accurate as illustrative examples but are no longer aspirational — read `babylon/README.md`
first if you're extending the real implementation rather than starting fresh.
...
- Quantities reflect the modelled geometry. A real hyperscale campus runs 20+ MV lineups and 20–36 generators; this represents one hall's worth of that facility.
