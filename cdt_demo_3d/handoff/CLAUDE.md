...
- 449 assets, 1,155 connections, 1,155 solved routes (17,720 m)
- 216 critical conduits, 188 hardwired or serial connections with no network path
...
- 11 CyHAZOPs hazards from WP09 (max cyber RPN 294), 16 MoR requirements, $8.88M baseline ALE
...
4. **The HTML files under the repo root are references.** three.js prototypes proving the
   data — read-only, never modified. The Babylon.js target viewer is built and lives at
   `babylon/hyperscale-campus-explorer.html` (see `babylon/README.md` for its status,
   architecture, and known gotchas); extend that file, not the three.js reference.
...
6. **`hazard-log.json` overrides inference.** Authored redundancy, timing, hazards, MoR and ALE
   come from the source papers. Do not recompute them from the model.
7. **Two inputs are unconfirmed** — the node crosswalk and the per-rack revenue basis. Every
   consequence figure depends on them; flag rather than rely.
8. **Re-solve routes after moving anything.** Edit `graph/positions.json`, run
