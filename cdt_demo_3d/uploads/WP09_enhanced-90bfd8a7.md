...
This chapter provides worked hazard log entries for six high-consequence CyHAZOPs nodes: UPS power conversion, central chiller facility, CDU/TCS, BMS supervisory, fire detection/suppression, and server BMC. The cross-node RPN ranking confirms that the SPOOFED guide word produces the highest cyber RPNs across all nodes — meaning telemetry integrity is the single most valuable security property in hyperscale OT. The CDU (Node N6) remains the highest-priority target, with a maximum cyber RPN of 294 and a cyber multiplier of 13.4× over its mechanical equivalent.
...
| Chiller facility optimiser | Staging, sequencing, setpoint | BACnet/IP to BMS | None | Wrong staging → insufficient capacity |
...
| N5-CY-005 | **REVERSE** | Staging logic | Chillers unloaded during peak demand | Attacker modifies facility optimiser demand signal | T0836 | Capacity removed when most needed; thermal excursion during peak AI training load | 8 | 4 | 6 | **192** | B | SL-2 |
...
| N5-CY-004 | Chiller facility and cooling towers on separate OT VLANs; compromise of one cannot reach other | P1 | $50K |
| N5-CY-005 | Manual chiller staging override physically available at facility room panel; not network-accessible | P2 | $20K |

### Vulnerability Research — Chiller Facility VFDs and Controllers

The following CVEs affect VFDs and chiller controllers commonly used in central facility cooling. These vulnerabilities directly enable the cyber causes in Table 9.6.

**Table 9.7a: Known CVEs — Chiller Facility VFDs and Controllers**
...
**Key Context:** The Johnson Controls Metasys CVE-2025-26385 (CVSS 10.0) is the highest-severity vulnerability affecting chiller facility control. York chillers integrated via Metasys ADS/ADX inherit this vulnerability. The Honeywell Niagara vulnerabilities (13 total, July 2025) enable full man-in-the-middle compromise of BMS controllers if encryption is disabled [Nozomi Networks, 2025].

### Standards Mapping — Node N5

**IEC 62443-3-2 Zone Assignment:** Node N5 assets belong to **Zone 1: BMS/HVAC** (chillers, AHUs, CRAHs, CDUs, pumps, VFDs). Recommended SL-T: SL 2 for field devices, SL 3 for chiller facility optimiser and BMS supervisory controllers [IEC 62443-3-2, Clause 5].
...
The CyHAZOPs analysis of Nodes N2, N5, and N6 demonstrates that telemetry integrity (SPOOFED guide word) produces the highest cyber RPNs across all nodes. The CDU (Node N6) remains the highest-priority target with a maximum cyber RPN of 294. The vulnerability research confirms that known CVEs in UPS NMCs (TLStorm), chiller facility VFDs (ABB, Siemens), and BMS platforms (Johnson Controls Metasys, Honeywell Niagara) directly enable the cyber causes identified in the hazard logs. Standards mapping per IEC 62443-3-2 assigns SL-T 3 to the most critical assets, requiring independent monitoring, hardwired interlocks, and protocol-level encryption. The protocol-level insecurity of BACnet/IP and Modbus TCP remains the single greatest systemic risk across all nodes.