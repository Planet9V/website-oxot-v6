...
| `positions.json` | Anchors | Bounding-box centre of all 450 asset meshes, in GLB world space |
| `facility-graph.json` | Facility | **209 assets / 314 connections** — substation, MV/LV, UPS, batteries, generators, BESS, chiller plant, towers, HVAC, MMR, core IT/DCS room, NOC, OT boundary, fire, security, internet boundary |
...
Every asset carries a `cell` field: `Cell A`, `Cell B` or `Campus (shared)`. 273 assets are cell-scoped (141 in Cell A, 132 in Cell B); 176 are shared campus plant.
...
**Shared campus:** substation and MV lineup, generators and bulk fuel, BESS, central chiller plant and cooling towers, carrier demarcs and the internet boundary, core IT / DCS room, NOC and campus BMS, and the data diode.
